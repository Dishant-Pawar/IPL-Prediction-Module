...
const Layout = ({ children, onLogout, theme, toggleTheme }) => {
  ...
  return (
    <div className="flex min-h-screen font-sans bg-[var(--bg-app)] text-[var(--text-normal)]">
      <aside className={`w-64 bg-[var(--sidebar-bg)] text-[var(--sidebar-text)] flex flex-col p-6 sticky top-0 h-screen shadow-2xl z-50 border-r border-white/5`}>
        ...
        <div className="mt-auto space-y-4 pt-6 border-t border-white/10">
           <button 
              onClick={toggleTheme}
              className="w-full flex items-center justify-center gap-2 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/50 hover:text-white transition-all shadow-inner"
           >
              <span className="material-symbols-outlined text-sm">{theme === 'light' ? 'dark_mode' : 'light_mode'}</span>
              Toggle {theme === 'light' ? 'Dark' : 'Light'} Perspective
           </button>
